...
<xsl:stylesheet xmlns="http://www.tei-c.org/ns/1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
 ...
 xmlns:alto="http://www.loc.gov/standards/alto/ns-v2#" xmlns:tei="http://www.tei-c.org/ns/1.0"
 ...
 <xsl:template match="alto:Page" mode="facsimile">
  <xsl:param name="imagepath" tunnel="yes"/>
  <surface type="page">
   <graphic url="{$imagepath}">
   </graphic>

  </surface>
 </xsl:template>
 
 <xsl:template match="alto:Page" mode="facsimile-full">
  <xsl:param name="imagepath" tunnel="yes"/>
  <surface type="page" xml:id="{@ID}" ulx="0" uly="0" lrx="{local:to-millimeter(@WIDTH)}"
   lry="{local:to-millimeter(@HEIGHT)}">
   <graphic>
